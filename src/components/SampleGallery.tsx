        ...
        <CarouselContent>
          {samples.map(({ img, title, lang, style }, idx) => (
            <CarouselItem key={idx}>
              <div
                className={`bg-gradient-to-br ${styleColors[style as keyof typeof styleColors]} rounded-2xl border-4 border-yellow-300 shadow-xl hover:shadow-2xl transition-all flex flex-col items-center p-6 relative comic-outline`}
              >
                <img src={img} alt={title} className="w-full max-w-xs h-56 object-cover rounded-lg border-4 border-pink-200 mb-3 shadow comic-outline animate-fade-in" />
